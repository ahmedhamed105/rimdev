...
public class ErrorlogController {
	...
	public @ResponseBody ResponseEntity<LogError> log_text(HttpServletRequest req, @RequestBody Logobject log,
		...
		try {
			...
			LogError result=logServ.errorlogexternal(devinfo.getDeviceip(), log.getRequesturl(), log.getLogtext(), devinfo,
					log.getUserid(), log.getLogtype(), langcode, log.getLogexception());

			return new ResponseEntity<LogError>(result, HttpStatus.OK);
		} catch (Exception e) {
